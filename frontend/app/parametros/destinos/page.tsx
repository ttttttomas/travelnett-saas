...
export default function DestinosPage() {
  ...
  const [destinosData, setDestinosData] = useState<Destino>({
    name: "",
    sigla: "",
  });
        ...
        <ul className="border border-gray-300 rounded-lg overflow-hidden divide-y divide-gray-200 bg-white">
          {isUpdating ? (
            <li className="py-12 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-sm text-gray-500 animate-pulse font-medium">Buscando destinos...</p>
            </li>
          ) : destinos.length === 0 ? (
        ...
        >
          <div className="flex flex-col gap-4">
            <input
              ...
              value={destinosData.name}
              onChange={(e) => setDestinosData({ ...destinosData, name: e.target.value })}
              className="w-full border bg-white rounded-sm p-2 pr-4 text-black/90 font-medium shadow-sm focus:outline-none"
            />
            <input
              type="text"
              placeholder="Sigla (ej: MDQ)"
              value={destinosData.sigla}
              onChange={(e) => setDestinosData({ ...destinosData, sigla: e.target.value })}
