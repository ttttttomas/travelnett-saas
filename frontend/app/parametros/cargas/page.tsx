...
export default function CargasPage() {
  ...
  const [lugares, setLugares] = useState<LoadingPlace[]>([]);
  const [cargaData, setCargaData] = useState<LoadingPlace>({
    name: "",
    address: "",
    type: "bus",
  });
  ...
  const handleClickPut = (lugar: LoadingPlace) => {
    setCargaData(lugar);
  ...
  const handleSubmitAdd = async () => {
    try {
      ...
      toast.error("Error al agregar el lugar de carga");
      console.error("Error creating loading place:", error);
    }
  };
  ...
  const handleSubmitPut = async () => {
    try {
      ...
      toast.error("Error al actualizar el lugar de carga");
      console.error("Error updating loading place:", error);
    }
  };
  ...
  const lugaresFiltrados = lugares
    .filter((e: any) => (e.type || e.tipo) === tipo)
    .filter((e: any) => {
    ...
    <Container>
      ...
      {!tipo ? (
        <section className="my-10 w-full max-w-3xl mx-auto">
          <ul className="text-white flex flex-col gap-8">
            <li
              onClick={() => setTipo("bus")}
              className="bg-primary cursor-pointer w-full gap-3 py-4 text-center px-3 rounded-lg font-bold text-xl hover:scale-105 transition-transform"
            >
              <i>BUS</i>
            </li>
            <li
        ...
        <>
          ...
          <div className="flex justify-center mb-6">
            <button
              onClick={() => {
                setCargaData({ name: "", address: "", type: tipo });
            ...
            <ul className="border border-gray-300 rounded-lg overflow-hidden divide-y divide-gray-200 bg-white">
              {isUpdating ? (
                <li className="py-12 flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-sm text-gray-500 animate-pulse font-medium">Buscando lugares...</p>
                </li>
      ...
      {(modalOpenAdd || modalOpenPut) && (
        <ModalLayout
          ...
          title={modalOpenAdd ? "Agregar Lugar de Carga" : "Editar Lugar de Carga"}
          svg={
