...
export default function RegimenesPage() {
  ...
  const [regimenesData, setRegimenesData] = useState<Regimen>({
    name: "",
    sigla: "",
    description: "",
  });
  ...
  const handleClickPut = (regimen: Regimen) => {
  ...
  const handleSubmitAdd = async () => {
    try {
      ...
      toast.error("Error al agregar el régimen");
  ...
  const handleSubmitPut = async () => {
    try {
      ...
      toast.error("Error al actualizar el régimen");
        ...
        <ul className="border border-gray-300 rounded-lg overflow-hidden divide-y divide-gray-200 bg-white">
          {isUpdating ? (
            <li className="py-12 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-sm text-gray-500 animate-pulse font-medium">Buscando régimenes...</p>
            </li>
          ) : regimenes.length === 0 ? (
      ...
      {modalOpenAdd && (
        ...
        >
          <div className="flex flex-col gap-4">
            <input
              type="text"
              placeholder="name del régimen"
              value={regimenesData.name}
              onChange={(e) => setRegimenesData({ ...regimenesData, name: e.target.value })}
              className="w-full border bg-white rounded-sm p-2 pr-4 text-black/90 font-medium shadow-sm focus:outline-none"
            />
            <input
              type="text"
              placeholder="Sigla (ej: MAP)"
              value={regimenesData.sigla}
              onChange={(e) => setRegimenesData({ ...regimenesData, sigla: e.target.value })}
            ...
            <input
              ...
              value={regimenesData.description}
              onChange={(e) => setRegimenesData({ ...regimenesData, description: e.target.value })}
              className="w-full border bg-white rounded-sm p-2 pr-4 text-black/90 font-medium shadow-sm focus:outline-none"
            />
          </div>
        </ModalLayout>
