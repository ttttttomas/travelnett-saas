...
export default function TransportePage() {
  ...
  const handleClickPut = (bus: TransportCompany) => {
  ...
  const handleSubmitPut = async () => {
    const { name, cuit, web, phone, type } = busData;
    try {
        ...
        >
          <div className="flex flex-col gap-4">
            <div className="flex flex-col items-center gap-3">
              ...
              />
              <p className="w-full border bg-white rounded-sm p-2 pr-4 text-black/90 font-medium shadow-sm focus:outline-none">Tipo: {tipo!.charAt(0).toUpperCase() + tipo?.slice(1)}</p>
              <input
                type="text"
                onChange={(e) => setBusData({ ...busData, cuit: Number(e.target.value) })}
                placeholder="CUIT"
                className="w-full border bg-white rounded-sm p-2 pr-4 text-black/90 font-medium shadow-sm focus:outline-none"
              />
              <input
                type="text"
                onChange={(e) => setBusData({ ...busData, web: e.target.value })}
                placeholder="Web"
                className="w-full border bg-white rounded-sm p-2 pr-4 text-black/90 font-medium shadow-sm focus:outline-none"
              />
              <input
                type="text"
                onChange={(e) => setBusData({ ...busData, phone: Number(e.target.value) })}
